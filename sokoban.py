...
def drawLevel(matrix_to_draw):
	...
	if myLevel.getSize()[0] > myEnvironment.size[0] / 36 or myLevel.getSize()[1] > myEnvironment.size[1] / 36:
		...
		if myLevel.getSize()[0] / myLevel.getSize()[1] >= 1:
			new_image_size = int(myEnvironment.size[0] / myLevel.getSize()[0])
		# If level's y size > level's x size then resize according to y axis
		else:
			new_image_size = int(myEnvironment.size[1] / myLevel.getSize()[1])
	...
	# Map images to characters used in level design 
...
def get_level_state(level_obj):
    matrix = level_obj.getMatrix()
    walls = set()
    boxes = set()
    goals = set()
    player = None
    ...
    return walls, frozenset(goals), player, boxes
